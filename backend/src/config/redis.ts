...
export class RedisService {
  ...
  // 🆕 Dipakai saat teardown (mis. Jest afterAll di test integration) supaya
  // koneksi TCP ke Redis benar-benar ditutup dan tidak jadi open handle yang
  // bikin proses Jest menggantung ("did not exit one second after test run").
  public static async disconnect(): Promise<void> {
    if (this.client) {
      try {
        await this.client.quit();
      } catch (err) {
        logger.warn('Error saat menutup koneksi Redis: %s', (err as Error).message);
      }
      this.client = null;
      this.isConnected = false;
    }
